...
def elo_calculator(
    ...
    entity_name (str): The name of the entity for which to calculate Elo ratings_features.
    ...
    # Sort DataFrame to ensure elo calculation is always correct
...
def aggregate_player_elos(
    player_data: pd.DataFrame, team_data: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    team_elo_playerbased = (
        player_data.groupby(["league", "gameid", "teamid", "date"])
        .agg(["mean"])
        .reset_index()
    )
    ...
    team_data = team_data.merge(
        team_elo_playerbased, how="left", on=["gameid", "league", "date", "teamid"]
    ).reset_index(drop=True)
    team_data.sort_values(
        by=["date", "league", "gameid", "teamid"], ascending=True, inplace=True
    )
...
def trueskill_model(
...
) -> Tuple[Optional[DataFrame], Optional[DataFrame], Dict[Any, Any]]:
    r"""
    ...
    earned_gold = (
        input_data.groupby(["league", "gameid", "date", "teamid"])["earned gpm"]
        .sum()
        .reset_index()
    )
...
def ewm_model(df: pd.DataFrame, entity: str) -> pd.DataFrame:
    ...
    red_side["red_side_ema_before"] = red_side.groupby(identity)["result"].transform(
        lambda x: x.ewm(halflife=5, ignore_na=True).mean().shift().bfill()
    )
    red_side["red_side_ema_after"] = red_side.groupby(identity)["result"].transform(
        lambda x: x.ewm(halflife=5, ignore_na=True).mean()
    )

    blue_side = df[df["side"] == "Blue"].copy()
    blue_side["blue_side_ema_before"] = blue_side.groupby(identity)["result"].transform(
        lambda x: x.ewm(halflife=5, ignore_na=True).mean().shift().bfill()
    )
    blue_side["blue_side_ema_after"] = blue_side.groupby(identity)["result"].transform(
        lambda x: x.ewm(halflife=5, ignore_na=True).mean()
    )
    ...
    win_rates = (
        merged.groupby([identity], as_index=False)
        .apply(lambda group: group.ffill())
        .reset_index(drop=True)
    )
    columns = [
        "blue_side_ema_before",
        "blue_side_ema_after",
        "red_side_ema_before",
        "red_side_ema_after",
    ]
    ...
    win_rates["opp_red_side_ema_before"] = oe.get_opponent(
        win_rates["red_side_ema_before"].to_list(), "team"
    )
    win_rates["opp_blue_side_ema_before"] = oe.get_opponent(
        win_rates["blue_side_ema_before"].to_list(), "team"
    )

    # Predict Win Probability By Side Win Rate
    win_rates["side_ema_before"] = np.where(
        win_rates["side"] == "Blue",
        win_rates["blue_side_ema_before"],
        win_rates["red_side_ema_before"],
    )
    win_rates["opp_side_ema_before"] = np.where(
        win_rates["side"] == "Blue",
    ...
    # Final Sort
    win_rates.sort_values(
        by=["date", "league", "gameid", "result"], ascending=True, inplace=True
    )
...
def egpm_model(data: pd.DataFrame, entity: str) -> pd.DataFrame:
    ...
    data["opp_egpm_dominance_ema_before"] = data[
        "opp_egpm_dominance_ema_before"
    ].fillna(0)
    data["opp_egpm_dominance_ema_after"] = data.groupby([identity])[
        "opp_egpm_dominance_ratio"
    ].transform(lambda x: x.ewm(halflife=5, ignore_na=True).mean())
    data["opp_egpm_dominance_ema_after"] = data["opp_egpm_dominance_ema_after"].fillna(
        0
    )

    data["egpm_dominance_win_perc"] = data["egpm_dominance_ema_before"] / (
        data["egpm_dominance_ema_before"] + data["opp_egpm_dominance_ema_before"]
    )
    data["egpm_dominance_win_perc"] = data["egpm_dominance_win_perc"].fillna(0.5)
    data["egpm_dominance_diff"] = (
        data["egpm_dominance_ema_before"] - data["opp_egpm_dominance_ema_before"]
    )
...
def dk_enrich(oe_data: pd.DataFrame, entity: str):
    ...
    if entity == "team":
        oe_data["dkpoints"] = (
            ...
            + (np.where((oe_data["gamelength"] / 60) < 30, 2, 0))
        )
    elif entity == "player":
        oe_data["dkpoints"] = (
            (3 * oe_data["kills"])
...
def enrich_ema_statistics(oe_data: pd.DataFrame, entity: str):
    ...
    for col in columns:
        oe_data[f"{col}_ema_after"] = oe_data.groupby([identity])[col].transform(
            lambda x: x.ewm(halflife=5, ignore_na=True).mean()
        )
        oe_data[f"{col}_ema_before"] = oe_data.groupby([identity])[col].transform(
            lambda x: x.ewm(halflife=9, ignore_na=True).mean().shift().bfill()
        )
